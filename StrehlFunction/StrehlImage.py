...
def PsfFit(grid,center=(0,0),max=1,dictionary={},full_answer=True):
  ...
  # UPDATE R99X
  (r99x,r99y),(r99u,r99v) = IF.EnergyRadius(grid,W.type["fit"],dic=res[0]) # the apertur radius
...
def Photometry(grid):
  ...
  # ELL AP
    ...
    bol   = IF.EllipticalAperture(W.Im0,dic={"center_x":x0,"center_y":y0,"ru":r99u,"rv":r99v,"theta":theta} )["bol"]
    ...
    W.strehl["my_photometry"] =  phot["sum"]- phot["number_count"]* W.strehl["my_background"]
  ...
  # MANUAL
     ...
     W.strehl["my_photometry"]=photometry - W.strehl["number_count"]* W.strehl["my_background"]
  ...
  W.strehl["eccentricity"] = np.sqrt( W.strehl["fwhm_a"]**2 - W.strehl["fwhm_b"]**2  )/W.strehl["fwhm_a"]

  W.strehl["snr"] = W.strehl["my_photometry"]/ W.strehl["my_background"] / np.sqrt(W.strehl["number_count"])
...
def Background(grid, param = {}):
  ...
  # 8 RECTS
    ...
    restmp= IF.EightRectangleNoise(grid,(xtmp-r99x,xtmp+r99x,ytmp-r99y,ytmp+r99y))
  ...
  # ELLIPTICAL ANNULUS
    ...
    bol_o   = IF.EllipticalAperture(cutted,dic={"center_x":myrad,"center_y":myrad,"ru":ruo,"rv":rvo,"theta":W.strehl["theta"]} )["bol"]
    bol_a = bol_o ^bol_i
...
def BinaryPsf(grid,search=False): # slowlyer
    ...
    if W.verbose >3 : print("Binary shapes :" , X.shape, Y.shape, IX.shape, eIX.shape)
    ...
    if (not "Gaussian" in fit_type) and (not "Moffat" in fit_type) :
       if W.verbose > 0  : print("WARNING : There is no bessel, None, and Gaussian hole fit type for binary  fi, fit type is set to gaussian")
       fit_type="Gaussian"


    if "Moffat" in fit_type :
       W.suposed_param['b0'],W.suposed_param['b1']=1.8,1.8
       if not '2D' in fit_type : doNotFit.append("b1")
       James['b0']=(1,10) ; James['b1']=(1,10)
    ...
    if W.verbose>3 : print( "fit type is : " ,fit_type)
    ...
    res = FF.leastsqFit(
           lambda x,y: vars(BF)[fit_type.replace("2D","")+"2pt"](x,y,dic=dic_for_fit),
    ...
    try : dic_copy0["exponent"] = dic_copy0["b0"]
    except : pass
    try : dic_copy1["exponent"] = dic_copy1["b1"]
    except : pass
...
def EllipseEventBack():
     ...
     ell_i   = IF.EllipticalAperture(W.Im0,dic={"center_x":obj.x0,"center_y":obj.y0,"ru":rui,"rv":rvi,"theta":obj.theta} ) # inner

     ell_o   = IF.EllipticalAperture(W.Im0,dic={"center_x":obj.x0,"center_y":obj.y0,"ru":ruo,"rv":rvo,"theta":obj.theta} ) #outter


     bol_a  = ell_o["bol"] ^  ell_i["bol"]  # annulus  inside out but not inside in
...
def EllipseEventPhot():
     ...
     ellipse_stat  =  IF.EllipticalAperture(obj.array,dic={"center_x":obj.x0,"center_y":obj.y0,"ru":obj.ru,"rv":obj.rv,"theta":obj.theta}, full_answer = True )  # ellipse , photometry
...
def AnnulusEventPhot(obj): # Called by Gui/Event...py  Event object
     ...
     res["my_photometry"] = res["phot"] - len(obj.array[bol_e])*res["my_background"]
