...
def SkyFormat( ra,dec ) : 
  ...
  if (ra == 99)   or (type(ra) ==str)  :  x = "N/A"
  else :          x = CT.decimal2hms(ra,":")
  if (dec == 99)  or (type(dec) ==str) :  y = "N/A"
  else         :  y =  CT.decimal2dms(dec,":")
  return x,y 
...
def Separation( point = (  (0,0),(0,0))   , err= ((0,0),(0,0))  ) : 
  ...
  sky_angle = np.arccos( angle[1]*W.north_direction[1] + angle[0]*W.north_direction[0])* 57.295779 # inverted angle and not north 
...
def PlotAnswer(unit=None,append=True): # CALLER  
  ...
  if (  (not  type(W.type["pick"]) is list) or  ( W.type["pick"][0]=='many' and W.type["pick"][1]==1 ) )  or (not append)  :
      ...
      G.bu_answer_type=Button(G.AnswerFrame,text='useless',background='Khaki' ,borderwidth=1,**G.bu_arg) 
      G.lb_answer_type=Label(G.AnswerFrame,text="useless",justify=LEFT,anchor="nw",**G.lb_arg)
  ...
  if W.type["pick"]=='one': PlotPickOne() 
  elif (W.type["pick"]=='binary')   or (W.type["pick"]=='tightbinary'): 
      PlotBinary() 
  elif W.type["pick"]=='ellipse': PlotEllipse() 
     ...
     if append : 
        ...
        DisplayAnswer(  row = 1  , 
                     font=tkFont.Font(size=6)   )
     return 
  elif W.type["pick"][0]=="stat": PlotStat()
...
def PlotPickOne():
  ...
  #W.strehl["strehl2_2"] = 100 *np.exp(-(rms*2*np.pi/2.17)**2) 
  W.strehl["err_strehl2_2"] = W.strehl["strehl2_2"]/W.strehl["strehl"]*W.strehl["err_strehl"]
  ...
  if G.scale_dic[0]["answer"]=="detector":
      ...
      W.tmp.lst=[  
         ...
         ["Eq. SR(2.17"+ u"\u03bc" +"m): ", W.strehl["strehl2_2"]     ,MyFormat(W.strehl["strehl2_2"],1,"f") +" +/- " +  MyFormat(W.strehl["err_strehl2_2"],1,"f") + " %"],
         ["Center x,y: "               , (W.strehl["center_y"],W.strehl["center_x"]) , MyFormat(W.strehl["center_y"],3,"f")    +" , "+   MyFormat(W.strehl['center_x'],3,"f")      ],  # need to inverse
         ["FWHM a,b,e: "           , (W.strehl["fwhm_x"],W.strehl["fwhm_y"]) , MyFormat(W.strehl["fwhm_a"],1,"f")   +", " +  MyFormat(W.strehl["fwhm_b"],1,"f") +", "+  MyFormat(W.strehl["eccentricity"],2,"f") + "[pxl]"  ], 
         ["Photometry: "           , W.strehl["my_photometry"]    ,MyFormat(W.strehl["my_photometry"],1,"f")  + " [adu]"],
         ["Background: "           , W.strehl["my_background"]    ,MyFormat(W.strehl["my_background"],1,"f") +'| rms: ' + MyFormat(W.strehl['rms'],1,"f")+ "[adu]" ], 
         ["S/N: "           , W.strehl["snr"]    ,MyFormat(W.strehl["snr"],1,"f") ],
         ["Peak: "            , W.strehl["intensity"]     ,MyFormat(W.strehl["intensity"],1,"f") +" [adu]"],
  ...
  ## SKY COORD 
      ...
      W.tmp.lst=[  
             ...
             ["Eq. SR(2.17"+u"\u03bc" +"m): ", W.strehl["strehl2_2"]     , "%.1f"% W.strehl["strehl2_2"]+ " +/- " +  MyFormat(W.strehl["err_strehl2_2"],1,"f") +  "%"],
            ["Center RA,Dec: "               , (W.strehl["center_ra"],W.strehl["center_dec"]) ,  "%s , %s"   % ( SkyFormat( W.strehl['center_ra'],  W.strehl['center_dec'] )   ) ], 
             ["FWHM a,b,e: "   , (W.strehl["fwhm_x"],W.strehl["fwhm_y"]) ,  "%.1f" % (W.strehl["fwhm_a"]*pxll*1000)   +", " +  "%.1f" % (W.strehl["fwhm_b"]*pxll*1000)   +", "+  "%.2f" % W.strehl["eccentricity"]  + "[mas]" ], 
             ["Photometry: "           , W.strehl["my_photometry"]    , "%.2f"% (W.head.zpt-2.5*np.log10(W.strehl["my_photometry"]/W.head.exptime))  + " [mag]"],
             ["Background: "           , W.strehl["my_background"]    , "%.2f"% (W.head.zpt-2.5*np.log10(W.strehl["my_background"]/W.head.exptime))  + '| rms: ' + "%.2f" % (W.head.zpt-2.5*np.log10(W.strehl['rms']) )  + " [mag]"],
             ["S/N: "           , W.strehl["snr"]    ,MyFormat(W.strehl["snr"],1,"f")  ],
...
def PlotEllipse():
  ...
  # BUTTON 
  G.AnswerFrame.columnconfigure(0,weight=1)  # to fill th ecolumn on th epossible space  
  G.bu_answer_type=Button(G.AnswerFrame,text='useless',background='Khaki' ,borderwidth=1,width=9) 
  G.lb_answer_type=Label(G.AnswerFrame,text="useless",justify=LEFT,anchor="nw",**G.lb_arg)
  ...
  if G.scale_dic[0]["answer"]=="detector":
      ...
      W.tmp.lst=[  
         ["Strehl: "               , W.strehl["strehl"]        ,MyFormat(W.strehl["strehl"],1,"f") +  " +/- "+   MyFormat(W.strehl["err_strehl"],1,"f")    +" %" ],
         ["Intensity: "            , W.strehl["intensity"]     ,MyFormat(W.strehl["intensity"],1,"f") +" [adu]"],
         ["Background: "           , W.strehl["my_background"]    ,MyFormat(W.strehl["my_background"],1,"f") +' +/- ' + MyFormat(W.strehl['rms'],1,"f")+ "[adu]" ], 
         ["Photometry: "           , W.strehl["my_photometry"]    ,MyFormat(W.strehl["my_photometry"],1,"f")  + " [adu]"],
         #["S/N: "           , W.strehl["snr"]    ,MyFormat(W.strehl["snr"],1,"f") ],
         ["Eq. SR(2.17"+u"\u03bc" +"m): ", W.strehl["strehl2_2"]     ,MyFormat(W.strehl["strehl2_2"],1,"f") +  " %"],
         ["Center x,y: "               , (W.strehl["center_y"],W.strehl["center_x"]) , MyFormat(W.strehl["center_y"],3,"f")    +" , "+   MyFormat(W.strehl['center_x'],3,"f")      ],  # need to inverse
         ]
  ...
  ## SKY COORD 
      ...
      try : 
        ...
        #   my_wcs = ProjectWcs(W.head.pywcs).all_pix2sky( np.array([[ W.strehl["center_y"],W.strehl["center_x"] ]]), 0 )
        #else : # not cube
           my_wcs = W.head.pywcs.all_pix2sky( np.array([[ W.strehl["center_y"],W.strehl["center_x"] ]]), 0 )
      except : 
        import traceback 
	...
	my_wcs= np.array( [ [99,99],[99,99] ]) 
      ...
      W.tmp.lst=[  
             ["Strehl: "               , W.strehl["strehl"]        , "%.1f"% (W.strehl["strehl"])+  " +/- "+"%.1f"% W.strehl["err_strehl"]+" %" ],
             ["Intensity: "            , W.strehl["intensity"],  "%.1f"% (W.head.zpt-2.5*np.log10(W.strehl["intensity"]/W.head.exptime))  +" [mag]"],
             ["Background: "           , W.strehl["my_background"]    , "%.2f"% (W.head.zpt-2.5*np.log10(W.strehl["my_background"]/W.head.exptime))  + '| rms: ' + "%.2f" % (W.head.zpt-2.5*np.log10(W.strehl['rms']) )  + " [mag]"],
             ["Photometry: "           , W.strehl["my_photometry"]    , "%.2f"% (W.head.zpt-2.5*np.log10(W.strehl["my_photometry"]/W.head.exptime))  + " [mag]"],
             #["S/N: "           , W.strehl["snr"]    ,MyFormat(W.strehl["snr"],1,"f")  ],
...
def PlotPickMany(append=True):
	 ...
	 # The last picked info, the seconline, the second column (with the float and no string) and x or y 
      ...
      if not append : W.type["pick"][1] -= 1  # To index well the strehl, after I increase it  see later 
      # 2/ answer list : 
      ##############
      ## 2.1 IMAGE COORD 
      lst =  [
	...
	# SEPARATION
      ...
      if not append : W.type["pick"][1] += 1  # To index well the strehl, I decreased it before, if you don't do, each push on button "to sky" leads to an increase in the indexation of the objet in the label but not in the arrows  
      ...
      if G.scale_dic[0]["answer"]=="detector":
          G.bu_answer_type["text"]=u"\u21aa"+'To sky     '
          G.bu_answer_type["command"]=lambda: PlotAnswer(unit="sky",append=False) 
          G.lb_answer_type["text"]= "In detector units"
      else : # including answer in sky 
          G.bu_answer_type["text"]=u"\u21aa"+'To detector'
          G.bu_answer_type["command"]=lambda: PlotAnswer(unit="detector",append=False) 
          G.lb_answer_type["text"]= "In sky units"
...
def PlotStat(): 
  ...
  for i in lst:
    print i[0] + i[1]
    G.ax3.text(0.3,1.0-float(num)/(len(lst)+1), i[0]+i[1],transform = G.ax3.transAxes ) 
...
def PlotStar():       #will also take W.sthrel["psf_fit"] 
  if (W.type["pick"]=="binary") or  (W.type["pick"]=="tightbinary"): # BINARY  draw radial Profile 
    PlotBinaryStar1D()
...
def PlotOneStar1D(): 
   ...
   ## PLOT radius profile 
   params =W.strehl      
   if W.verbose >3 : print 'center=',center
   ...
   a = np.arange(min(x),max(x),0.1) # we get a smaller bin for the fitted curve.  
   # RAW  DATA in X
   G.ax2.plot(x+0.5,y,color='black',linestyle='steps',linewidth=1,label='Data')  # x+0.5 to recenter the bar
   ...
   if not W.head.wavelength*1e-6/W.head.diameter/(W.head.pixel_scale/206265)<2:  
      ...
      G.ax2.plot(a,MyBessel+params['my_background'],color='blue',linewidth=2,label='Ideal PSF')
                                                                         # Legend
...
def PlotBinaryStar1D():
   ...
   if "Moffat" in W.type["fit"] : fit_type="Moffat2pt"
   ...
   if W.type["fit"] != None : 
      I_theory =vars(BF) [ fit_type  ] ( (x_theory,y_theory) ,W.strehl["fit_dic"])
   else : I_theory = 0*x_theory 
...
def PlotOneStar2D(): 
    ...
    # NOISE ANNULUS 
       ...
       for rt in lst :  
         ...
         a= matplotlib.patches.Ellipse(   (x,y),width,height,angle,fc="none",ec="yellow", linestyle = "solid"  , alpha = 0.6  ) 
         G.ax32.add_patch(a)
    ...
    if W.type["phot"] == "encircled_energy":
       ...
       a= matplotlib.patches.Rectangle((ty-params['r99y'],tx-params['r99x'] ),2*params['r99y'],2*params['r99x'], facecolor='none',edgecolor='black')
       G.ax32.add_patch(a)
    ...
    #PHOT ELL
       ...
       x =  params["center_y"] - r[2] 
       y =  params["center_x"] - r[0] 
       a= matplotlib.patches.Ellipse(   (x,y),width,height,angle,fc="none",ec="black"  ) 
       G.ax32.add_patch(a)
...
def PlotBinaryStar2D():  
    ...
    G.ax31.format_coord=lambda x,y:"" 
      # FIT 
    if "Moffat" in W.type["fit"]: stg = "Moffat2pt"
    elif "Gaussian" in W.type["fit"]: stg = "Gaussian2pt"
    G.ax32=G.figresult.add_subplot(122)                   
...
def CallContrastMap():    
  ...
  Label(G.ContrastButton1Frame,text="Peak Intensity",**G.lb_arg).grid(row=0,column=0,sticky="snew") 
