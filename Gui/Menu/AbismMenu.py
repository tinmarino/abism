...
def AppearanceMenu(parent_menu, who="bg", label="Background"):
    ...
    vars(G)["cu_appearance_"+who] = StringVar()
    vars(G)["cu_appearance_"+who].set(vars(G)[who][0])
...
def PanedConfigMenu(parent_menu, label=""):
    ...
    def CrazyColor():
        import color_interactive
        def tmp(x): return PanedConfig({"bg": x})
        color_interactive.Main(func=tmp)

    G.cu_p1 = StringVar()
    G.cu_p1.set(G.paned_dic["sashwidth"])
    G.cu_p2 = StringVar()
    G.cu_p2.set(G.paned_dic["sashrelief"])
    G.cu_p3 = StringVar()
    G.cu_p3.set(G.paned_dic["bg"])
    ...
    for j in [[1, "sashwidth"], [3, "sashrelief"], [2, "bg"]]:
        for i in locals()["lst"+str(j[0])]:
            ...
            if i == "\n":
                ...
                pconfig_menu.add_command(
                    label=i[1], bg=None, state=DISABLED)

            elif "More" in str(i[0]):
                pconfig_menu.add_command(
                    label="More color", command=CrazyColor)
            else:
                pconfig_menu.add_radiobutton(
                    label=i[0],
                    command=lambda k=k: PanedConfig(
                        {k[1][1]: k[0][1]}),  # k=(i,j)
...
def AppearanceMenu1():
    ...
    more_lst = [
        ["Color", lambda: AppearanceMenu(
            more_menu, who="bg", label="Background Color")],  # background color
        ["Color", lambda: AppearanceMenu(
            more_menu, who="fg", label="Foreground Color")],  # foreground color
        ["Color", lambda: PanedConfigMenu(
            more_menu, label="Config Sash")],
    ]
    for i in more_lst:
        i[1]()
    G.abism_menu.menu.add_cascade(
        menu=more_menu, label="Appearance", underline=0)
    return
