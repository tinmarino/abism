...
def ViewMenu(root, parent, args):
    ...
    def Color():
        ...
        if G.scale_menu_type == "cascade":
            color_menu = tk.Menu(G.scale_menu, **skin().fg_and_bg)
        else:
        ...
        for i in lst:
            color_menu.add_radiobutton(
                label=i[0], command=lambda i=i: root.ImageFrame.CutImageScale(
        ...
        color_menu.add_command(
            label='Contour', command=lambda: root.ImageFrame.CutImageScale(
        ...
        for i in all_cmaps:
            ...
            more_color_menu.add_radiobutton(
                label=i,
                command=lambda i=i: root.ImageFrame.CutImageScale(dic={"cmap": i}),
    ...
    def Scale():
        """Scale of image drop"""
        if G.scale_menu_type == "cascade":
            scale_menu = tk.Menu(G.scale_menu, **skin().fg_and_bg)
        else:
    ...
    def Cut():
        ...
        if G.scale_menu_type == "cascade":
            cut_menu = tk.Menu(G.scale_menu, **skin().fg_and_bg)
        else:
