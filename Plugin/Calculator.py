...
class MyCalculator:
    def __init__(self, frame):
        ...
        # for the buttons not the answer
        self.top_frame = Frame(self.frame, **G.fr_arg)
        for i in range(5):
            self.top_frame.columnconfigure(
                i, weight=1)  # to expand the buttons

        if W.verbose > 2:
            print("cal color ", self.top_frame["bg"])
    ...
    def CreateCal(self):
        ...
        for b in btn_list:
            rel = 'raised'
            def cmd(x=b): return self.Click(x)
