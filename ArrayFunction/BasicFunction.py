...
def Gaussian2pt(points, params,dic={"aniso":1,"same_psf":1}):
  ...
  if dic["aniso"]==0:
     ...
     if dic["same_psf"]==1:
        res= I0*np.exp(- ((x-x0)**2+(y-y0)**2)/a0**2 ) + I1*np.exp(- ((x-x1)**2+(y-y1)**2)/a0**2 )+ bck
     else :# including same_fit =0
        res = I0*np.exp(- ((x-x0)**2+(y-y0)**2)/a0**2 ) + I1*np.exp(- ((x-x1)**2+(y-y1)**2)/a1**2 )+ bck
   ...
   #           xp(-(xp**2/params['spread_x']**2+yp**2/params['spread_y']**2))
        res= I0*np.exp(  - (x0p/a0x)**2 - (y0p/a0y)**2 )   + I1*np.exp(- (x1p/a0x)**2 - (y1p/a0y)**2 )  +   bck
     else :# including same_fit =0
        res= I0*np.exp(  - (x0p/a0x)**2 - (y0p/a0y)**2 )   + I1*np.exp(- (x1p/a1x)**2 - (y1p/a1y)**2 )  +   bck
  return res
...
def Gaussian2D(xy,params):
    ...
    res = params['background']+params['intensity']*np.exp(-(xp**2/params['spread_x']**2+yp**2/params['spread_y']**2))
    return res
...
def Gaussian_hole(points,params):    # If there is a negative gaussian in the center, a hole
    ...
    if params["same_center"]==1: x0h,y0h,th=x0,y0,t
    else : x0h,y0h,th=params['center_x_hole'],params['center_y_hole'],params['theta_hole']
    if params["2D"]==0:params['spread_y'],params['spread_y_hole']=params['spread_x'],params['spread_x_hole']
    ...
    res = params['background']+params['intensity']*np.exp(-(xp**2/params['spread_x']**2+yp**2/params['spread_y']**2))
    res-= params["intensity_hole"]*np.exp(-(xh**2/params['spread_x_hole']**2+yp**2/params['spread_y_hole']**2))
    return res
...
def Bessel1(points,params):
  ...
  I=  params['intensity']
  cst=params['background']
  r = np.sqrt( ((x-x0)**2 +(y-y0)**2)/a**2 )  #nb : r is directly divided by a
...
def Bessel12D(xy,params) :
    ...
    xp = (xt-params['center_x'])*np.cos(params['theta'])-(yt-params['center_y'])*np.sin(params['theta'])
    yp = (xt-params['center_x'])*np.sin(params['theta'])+(yt-params['center_y'])*np.cos(params['theta'])
    r = np.sqrt( xp**2/params['spread_x']**2 + yp**2/params['spread_y']**2 )  #nb : r is directly divided by a
...
def DiffractionPatern(points,params):
  ...
  if not e==0: #otherwise division by 0
     res =  np.nan_to_num(  ( 2*jn(1,u)/u - e**2 *2*jn(1,e*u)/e/u  )**2  ) #+ np.float_(np.array(r)==0)
  else :
